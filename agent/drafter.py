...
_DRAFT_SYSTEM = (
    ...
    "3. 각 항목은 3~6문장의 서술형 문단으로 쓴다. 개조식 나열이나 표는 쓰지 마라.\n"
    "4. '~한다', '~이다' 같은 문어체 평서형으로 쓴다. 신청서 문체다.\n"
    "5. 요청받은 항목만, 요청받은 제목 그대로 sections에 담아라."
)
...
def generate(notice: Notice, profile: CompanyProfile,
    ...
    if hyperclova_api.is_configured():
        ...
        try:
            data = hyperclova_api.chat_structured(_DRAFT_SYSTEM, user, _DRAFT_SCHEMA,
                                                  max_tokens=2000)
            raw_sections = [(str(s.get("title", "")).strip(), str(s.get("body", "")).strip())
                            for s in data.get("sections", [])
                            if str(s.get("body", "")).strip()]
        except Exception as e:
            print(f"[알림] 초안 생성 LLM 호출 실패 — 뼈대 초안으로 대체합니다. ({e})")
            note = f"AI 호출에 실패해 뼈대 초안만 넣었습니다. ({e})"
    else:
